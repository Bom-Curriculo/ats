...
def get_provider_chain() -> list[str]:
    ...
    if selecionado != "auto":
        if selecionado not in PROVEDORES_SUPORTADOS and selecionado != "mock":
            raise AIProviderError(f"Provedor '{selecionado}' não reconhecido.")
        return [selecionado]
...
def is_provider_configured(nome: str) -> bool:
...
def _erro_seguro(nome: str, modelo: str | None, erro: Exception) -> SanitizedProviderError:
...
def _ia_habilitada(solicitacao: AnalysisRequest) -> bool:
    """A opção explícita da requisição prevalece sobre o default do ambiente."""
    if solicitacao.use_ai is not None:
        return solicitacao.use_ai
...
async def run_analysis_with_fallback(
    ...
    fabrica: Callable[[str], AIProvider] = create_provider,
) -> AnalysisResult:
    """Executa análise local e filtra"""

    resultado = analyze_resume(solicitacao)
    curriculo = sanitize_personal_data(solicitacao.resume_text)
    vaga = sanitize_personal_data(solicitacao.job_text)
    itens_removidos = list(
        dict.fromkeys(curriculo.itens_removidos + vaga.itens_removidos)
    )
    segura = solicitacao.model_copy(
        update={
            "resume_text": curriculo.texto_sanitizado,
            "job_text": vaga.texto_sanitizado,
            "resume_sources": [],
        }
    )
    ...
    erros: list[str] = []
    detalhes_erros: list[SanitizedProviderError] = []


    for indice, nome in enumerate(cadeia):
        if not is_provider_configured(nome):
            ignorados.append(nome)
            continue


        tentados.append(nome)
        provedor: AIProvider | None = None
        try:
