...
def technologies_in_text(texto: str) -> list[str]:
    return [t.nome for t in CATALOGO if contains_alias(texto, t.aliases, t.nome)]


def _looks_like_title(linha: str, proxima: str = "") -> bool:
...
def extract_projects(texto: str, *, tipo_fonte: str = "projeto") -> list[dict]:
    linhas = [x.strip() for x in texto.splitlines() if x.strip()]
    if not linhas:
        return []

    inicios = [i for i, linha in enumerate(linhas)
               if _looks_like_title(linha, linhas[i + 1] if i + 1 < len(linhas) else "")]
    if not inicios:
        return [{"nome": None, "tecnologias": technologies_in_text(texto), "descricao": texto[:2000],
    ...
    for pos, inicio in enumerate(inicios):
        ...
        projetos.append({"nome": nome, "tecnologias": technologies_in_text(corpus),
...
def section_block(texto: str, fonte: str, confianca: int = 90) -> list[dict]:
    if not texto.strip():
        return []
