...
def _classificar_url(valor: str, rotulo_portfolio: bool = False) -> str:
    limpa = valor.rstrip(".)]}>\"'")
...
def sanitizar_dados_pessoais(texto: str, *, remover_links: bool = True, remover_endereco: bool = True) -> ResultadoSanitizacao:
    ...
    if remover_links:
        ...
        def remover_url(match: re.Match[str]) -> str:
            ...
            tipo = _classificar_url(valor, valor.rstrip(".)]}>\"'") in portfolios)
            links[tipo] += 1
    ...
    return ResultadoSanitizacao(texto_sanitizado, encontrados, dict(links))
