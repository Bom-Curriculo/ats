...
class ClassificacaoVagaIA(BaseModel):
...
class EvidenciaSelecionada(BaseModel):
    item: str
    fonte: str | None = None
...
class AvaliacaoContextualRequisito(BaseModel):
    item: str
    importancia: str = "nao_informado"
    relevancia_para_vaga: str = "media"
    status: str = "nao_avaliado"
    evidencia_usada: EvidenciaSelecionada | None = None
...
class ResultadoPipelineIA(BaseModel):
    classificacao_vaga: ClassificacaoVagaIA | None = None
    evidencias_relevantes: list[EvidenciaSelecionada] = Field(default_factory=list)
    avaliacao_requisitos: list[AvaliacaoContextualRequisito] = Field(default_factory=list)
